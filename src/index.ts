...
export default class XlsxTemplate {
    ...
    */
    public deleteSheet(sheetName: number): XlsxTemplate {
        let sheet = this.loadSheet(sheetName);

        let sh = this.workbook.find("sheets/sheet[@sheetId='" + sheet.id + "']");
        this.workbook.find("sheets").remove(sh);

        let rel = this.workbookRels.find("Relationship[@Id='" + sh.attrib['r:id'] + "']");
    ...
    */
    public copySheet(sheetName: number, copyName: string): XlsxTemplate {
        let sheet = this.loadSheet(sheetName); //filename, name , id, root
        let newSheetIndex = (this.workbook.findall("sheets/sheet").length + 1).toString();
        let fileName = 'worksheets' + '/' + 'sheet' + newSheetIndex + '.xml';
        let arcName = this.prefix + '/' + fileName;

        this.archive.file(arcName, tostring(sheet.root, {}));
        this.archive.file(arcName).options.binary = true;

        let newSheet = SubElement(this.workbook.find('sheets'), 'sheet');
        ...
        let newRel = SubElement(this.workbookRels, 'Relationship');
    ...
    */
    private _rebuild() {
            ...
            .sort((rel1, rel2) => { //using order
                ...
                if ((index1 + index2) == 0) {
                    if (rel1.attrib.Id && rel2.attrib.Id)
                        return parseInt(rel1.attrib.Id.substring(3)) - parseInt(rel2.attrib.Id.substring(3));
        ...
        this.archive.file(this.prefix + '/' + '_rels' + '/' + path.basename(this.workbookPath) + '.rels', tostring(this.workbookRels, {}));
    ...
    public loadTemplate(data: Buffer | string) {
        ...
        // Load relationships
        let rels = parse(this.archive.file("_rels/.rels").asText()).getroot(),
        ...
        this.workbookRels = parse(this.archive.file(this.prefix + "/" + '_rels' + "/" + path.basename(workbookPath) + '.rels').asText()).getroot();
        this.sheets = this.loadSheets(this.prefix, this.workbook, this.workbookRels);
        ...
        this.sharedStrings = [];
        parse(this.archive.file(this.sharedStringsPath).asText()).getroot().findall('si').forEach((si) => {
            let t = { text: '' };
        ...
        let jpgType = this.contentTypes.find('Default[@Extension="jpg"]');
        if (jpgType === null) {
    ...
    public substitute(sheetNameOrIndex: string | number, substitutions: Object) {
        let sheet = this.loadSheet(sheetNameOrIndex);
        this.sheet = sheet;
        ...
        let dimension = sheet.root.find("dimension"),
            ...
            namedTables = this.loadTables(sheet.root, sheet.filename),
            rows = [],
            drawing = null;

        let rels = this.loadSheetRels(sheet.filename);
        sheetData.findall("row").forEach((row) => {
            ...
            row.findall("c").forEach((cell) => {
                ...
                if (cell.attrib.t === "s") {
                    ...
                    let cellValue = cell.find("v"),
                        stringIndex = parseInt(cellValue.text.toString(), 10),
                        string: string = this.sharedStrings[stringIndex];
                    ...
                    this.extractPlaceholders(string).forEach((placeholder) => {
                        ...
                        if (placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                            ...
                            if (rels != null) {
                                ...
                                this.substituteImage(cell, string, placeholder, substitution, drawing);
                            } else {
                                console.log("Need to implement initRels. Or init this with Excel")
                            }
                        } else {
                            ...
                            string = this.substituteScalar(cell, string, placeholder, substitution).toString();
                        }
                    });
                }
            ...
            if (newTableRows.length > 0) {
                ...
                if (this.option["moveImages"] && rels) {
                    ...
                    if (drawing != null) {
                        this.moveAllImages(drawing, parseInt(row.attrib.r), newTableRows.length)
                    }
                }
        ...
        if (dimension) {
            if (totalRowsInserted > 0 || totalColumnsInserted > 0) {
                let dimensionRange = this.splitRange(dimension.attrib.ref),
                    dimensionEndRef = this.splitRef(dimensionRange.end);
    ...
    public writeSharedStrings(): void {
        let root = parse(this.archive.file(this.sharedStringsPath).asText()).getroot(),
    ...
    public addSharedString(s: string): number {
        let idx = this.sharedStrings.length;
    ...
    protected loadSheet(sheetNameOrIndex: string | number): Sheet {
        ...
        for (let i = 0; i < this.sheets.length; ++i) {
            if ((typeof (sheetNameOrIndex) === "number" && this.sheets[i].id === sheetNameOrIndex) || (this.sheets[i].name === sheetNameOrIndex)) {
                info = this.sheets[i];
                break;
            }
        }
    ...
    protected loadSheetRels(sheetFilename: string): Sheet {
        ...
        let rels = { filename: relsFilename, root: parse(relsFile.asText()).getroot() }
        return rels;
    }
    ...
    protected initSheetRels(sheetFilename: string): Sheet {
        let sheetDirectory = path.dirname(sheetFilename),
            sheetName = path.basename(sheetFilename),
            relsFilename = path.join(sheetDirectory, '_rels', sheetName + '.rels').replace(/\\/g, '/');
        let root = Element('Relationships');
        root.set('xmlns', 'http://schemas.openxmlformats.org/package/2006/relationships');
        let relsEtree = new ElementTree(root);
        let rels = { filename: relsFilename, root: relsEtree.getroot() }
        return rels;
    }
    ...
    private loadDrawing(sheet: any, sheetFilename: string, rels: any): any {
        let sheetDirectory = path.dirname(sheetFilename),
            sheetName = path.basename(sheetFilename),
            drawing: any = { filename: '', root: null };
        let drawingPart = sheet.find("drawing");
        if (drawingPart === null) {
            drawing = this.initDrawing(sheet, rels);
            return drawing;
        }
        let relationshipId = drawingPart.attrib['r:id'],
            target = rels.find("Relationship[@Id='" + relationshipId + "']").attrib.Target,
            drawingFilename = path.join(sheetDirectory, target).replace(/\\/g, '/'),
            drawingTree = parse(this.archive.file(drawingFilename).asText());
    ...
    public initDrawing(sheet: any, rels: any): any {
        let maxId = this.findMaxId(rels, 'Relationship', 'Id', /rId(\d*)/);
        let rel = SubElement(rels, 'Relationship');
        ...
        let drawing: any = {};
        let drawingFilename = 'drawing' + this.findMaxFileId(/xl\/drawings\/drawing\d*\.xml/, /drawing(\d*)\.xml/) + '.xml';
    ...
    private _moveImage(drawingElement: Element, fromRow: number, nbRow: number) {
        let from = Number.parseInt(drawingElement.find('xdr:from').find('xdr:row').text.toString(), 10) + nbRow
        drawingElement.find('xdr:from').find('xdr:row').text = from
        let to = Number.parseInt(drawingElement.find('xdr:to').find('xdr:row').text.toString(), 10) + nbRow
    ...
    public loadTables(sheet: Element, sheetFilename: string): NamedTable[] {
        let sheetDirectory = path.dirname(sheetFilename),
            ...
            relsFile = this.archive.file(relsFilename),
            tables: NamedTable[] = []; // [{filename: ..., root: ....}]

        if (relsFile === null) {
            return tables;
        }

        let rels = parse(relsFile.asText()).getroot();

        sheet.findall("tableParts/tablePart").forEach((tablePart: { attrib: { [x: string]: any; }; }) => {
            let relationshipId = tablePart.attrib['r:id'],
                target = rels.find("Relationship[@Id='" + relationshipId + "']").attrib.Target,
                tableFilename = target.replace('..', this.prefix),
                tableTree = parse(this.archive.file(tableFilename).asText());
    ...
    public substituteTableColumnHeaders(tables: NamedTable[], substitutions: any): void {
        tables.forEach((table) => {
            let root = table.root,
                ...
                tableRange = this.splitRange(root.attrib.ref),
                idx = 0,
                inserted = 0,
                newColumns = [];
            ...
            let tableRoot = table.root,
                tableStart = this.splitRef(tableRange.start),
                tableEnd = this.splitRef(tableRange.end);
    ...
    public extractPlaceholders(string: string) {
    ...
    public isWithin(ref: string, startRef: string, endRef: string) {
        let start = this.splitRef(startRef),
            end = this.splitRef(endRef),
            target = this.splitRef(ref);

        return (
            start.row <= target.row
            && target.row <= end.row
            && start.colNo <= target.colNo
            && target.colNo <= end.colNo
        );
    ...
    public substituteScalar(cell: Element, string: string, placeholder: { full: any; key?: any; name?: string; placeholder: any; type?: string; }, substitution: SubstitutionValue) {
        if (placeholder.full) {
    ...
    public substituteTable(row: Element, newTableRows: any[], cells: any[], cell: Element, namedTables: any[], substitution: any[], key: any, placeholder: { subType: string; placeholder: any; }, drawing: any) {
        ...
        if (substitution.length === 0) {
            ...
            let parentTables = namedTables.filter((namedTable: { root: { attrib: { ref: any; }; }; }) => {
                let range = this.splitRange(namedTable.root.attrib.ref);
                return this.isWithin(cell.attrib.r, range.start, range.end);
            });

            substitution.forEach((element: any, idx: number) => {
                let newRow: Element, newCell: Element,
                ...
                if (idx === 0) { // insert in the row where the placeholders are
                    ...
                    parentTables.forEach((namedTable: { root: any; }) => {
                        let tableRoot = namedTable.root,
                            ...
                            range = this.splitRange(tableRoot.attrib.ref);
    ...
    public substituteImage(cell: Element, string: string, placeholder: any, substitution: SubstitutionValue, drawing: { relRoot: Element; root: Element; }) {
        ...
        //get max refid
        //update rel file.
        let maxId = this.findMaxId(drawing.relRoot, 'Relationship', 'Id', /rId(\d*)/);
        let maxFileId = this.findMaxFileId(/xl\/media\/image\d*.jpg/, /image(\d*)\.jpg/);
        let rel = SubElement(drawing.relRoot, 'Relationship');
        rel.set('Id', 'rId' + maxId);
        ...
        //put image to media.
        this.archive.file('xl/media/image' + maxFileId + '.jpg', this._toArrayBuffer(buffer), { binary: true, base64: false });
        ...
        this.sheet.root.findall("mergeCells/mergeCell").forEach((mergeCell: any) => {
            ...
            if (this.cellInMergeCells(cell, mergeCell)) {
                let mergeCellWidth = this.getWidthMergeCell(mergeCell, this.sheet)
                let mergeCellHeight = this.getHeightMergeCell(mergeCell, this.sheet)
                let mergeWidthEmus = this.columnWidthToEMUs(mergeCellWidth);
                let mergeHeightEmus = this.rowHeightToEMUs(mergeCellHeight);
        ...
        let imagePart = SubElement(drawing.root, 'xdr:oneCellAnchor');
        let fromPart = SubElement(imagePart, 'xdr:from');
        let fromCol = SubElement(fromPart, 'xdr:col');
        fromCol.text = (this.charToNum(this.splitRef(cell.attrib.r).col) - 1).toString();
        let fromColOff = SubElement(fromPart, 'xdr:colOff');
        fromColOff.text = '0';
        let fromRow = SubElement(fromPart, 'xdr:row');
        fromRow.text = (this.splitRef(cell.attrib.r).row - 1).toString();
        let fromRowOff = SubElement(fromPart, 'xdr:rowOff');
        fromRowOff.text = '0';
        let extImagePart = SubElement(imagePart, 'xdr:ext', { cx: imageWidth.toString(), cy: imageHeight.toString() });
        let picNode = SubElement(imagePart, 'xdr:pic');
        let nvPicPr = SubElement(picNode, 'xdr:nvPicPr');
        let cNvPr = SubElement(nvPicPr, 'xdr:cNvPr', { id: maxId.toString(), name: 'image_' + maxId, descr: '' });
        let cNvPicPr = SubElement(nvPicPr, 'xdr:cNvPicPr');
        let picLocks = SubElement(cNvPicPr, 'a:picLocks', { noChangeAspect: '1' })
        let blipFill = SubElement(picNode, 'xdr:blipFill');
        let blip = SubElement(blipFill, 'a:blip', {
            "xmlns:r": "http://schemas.openxmlformats.org/officeDocument/2006/relationships",
            "r:embed": "rId" + maxId
        });
        let stretch = SubElement(blipFill, 'a:stretch');
        let fillRect = SubElement(stretch, 'a:fillRect');
        let spPr = SubElement(picNode, 'xdr:spPr');
        let xfrm = SubElement(spPr, 'a:xfrm');
        let off = SubElement(xfrm, 'a:off', { x: "0", y: "0" });
        let ext = SubElement(xfrm, 'a:ext', { cx: imageWidth.toString(), cy: imageHeight.toString() });
        let prstGeom = SubElement(spPr, 'a:prstGeom', { 'prst': 'rect' });
        let avLst = SubElement(prstGeom, 'a:avLst');
        let clientData = SubElement(imagePart, 'xdr:clientData');
        return true;
    }

    // Clone an element. If `deep` is true, recursively clone children
    public cloneElement(element: Element, deep?: boolean) {
        let newElement = Element(element.tag.toString(), element.attrib);
    ...
    public getCurrentCell(cell: Element, currentRow: number, cellsInserted: number) {
        let colRef = this.splitRef(cell.attrib.r).col,
            colNum = this.charToNum(colRef);
    ...
    public updateRowSpan(row: Element, cellsInserted: number) {
        if (cellsInserted !== 0 && row.attrib.spans) {
            let rowSpan = row.attrib.spans.split(':').map((f: string) => parseInt(f, 10));
    ...
    public pushRight(workbook: Element, sheet: Element, currentCell: string, numCols: number) {
        let cellRef = this.splitRef(currentCell),
            currentRow = cellRef.row,
            currentCol = this.charToNum(cellRef.col);
        ...
        sheet.findall("mergeCells/mergeCell").forEach((mergeCell: Element) => {
            let mergeRange = this.splitRange(mergeCell.attrib.ref),
                mergeStart = this.splitRef(mergeRange.start),
                mergeStartCol = this.charToNum(mergeStart.col),
                mergeEnd = this.splitRef(mergeRange.end),
                mergeEndCol = this.charToNum(mergeEnd.col);
        ...
        workbook.findall("definedNames/definedName").forEach((name: { text: any; }) => {
            ...
            if (this.isRange(ref)) {
                let namedRange = this.splitRange(ref),
                    namedStart = this.splitRef(namedRange.start),
                    namedStartCol = this.charToNum(namedStart.col),
                    namedEnd = this.splitRef(namedRange.end),
                    namedEndCol = this.charToNum(namedEnd.col);
                ...
                let namedRef = this.splitRef(ref),
                    namedCol = this.charToNum(namedRef.col);
        ...
        sheet.findall("mergeCells/mergeCell").forEach((mergeCell: Element) => {
            ...
            if (mergeStart.row == currentRow) {
                for (let i = 1; i <= numRows; i++) {
                    let newMergeCell = this.cloneElement(mergeCell);
                    mergeStart.row += 1;
                    mergeEnd.row += 1;
    ...
    public getWidthMergeCell(mergeCell: Element, sheet: Sheet) {
        let mergeWidth = 0;
        let mergeRange = this.splitRange(mergeCell.attrib.ref),
            mergeStartCol = this.charToNum(this.splitRef(mergeRange.start).col),
            mergeEndCol = this.charToNum(this.splitRef(mergeRange.end).col);
    ...
    public getHeightCell(numRow: number, sheet: Sheet) {
        let defaultHight = sheet.root.find("sheetFormatPr").attrib["defaultRowHeight"]
        let finalHeight = defaultHight;
        sheet.root.findall("sheetData/row").forEach((row: Element) => {
            if (numRow === parseInt(row.attrib["r"])) {
    ...
    public getHeightMergeCell(mergeCell: Element, sheet: Sheet) {
        let mergeHeight = 0;
        let mergeRange = this.splitRange(mergeCell.attrib.ref),
            mergeStartRow = this.splitRef(mergeRange.start).row,
            mergeEndRow = this.splitRef(mergeRange.end).row;
    ...
    public cellInMergeCells(cell: Element, mergeCell: Element) {
        let cellCol = this.charToNum(this.splitRef(cell.attrib.r).col);
        let cellRow = this.splitRef(cell.attrib.r).row;
        let mergeRange = this.splitRange(mergeCell.attrib.ref),
            mergeStartCol = this.charToNum(this.splitRef(mergeRange.start).col),
            mergeEndCol = this.charToNum(this.splitRef(mergeRange.end).col),
            mergeStartRow = this.splitRef(mergeRange.start).row,
    ...
    public imageToBuffer(imageObj: SubstitutionValue) {
        ...
        if (imageObj instanceof Buffer) {
            ...
            if (typeof (imageObj) === 'string' || imageObj instanceof String) {
                ...
                //}
                try {
                    let buff = Buffer.from(imageObj, 'base64')
                    return buff;
                } catch (error) {
    ...
    public findMaxId(element: { findall: (arg0: any) => any[]; }, tag: string, attr: string, idRegex: RegExp) {
        let maxId = 0;
        element.findall(tag).forEach((element: { attrib: { [x: string]: any; }; }) => {
            let match = idRegex.exec(element.attrib[attr]);
            if (match == null) {
